#!/usr/bin/env python3
"""
Francis — Phase 3.5 (Ultra)
Advanced backfill for Phase 0/1/2 and upgrades on Phase 3 in ONE file.

Highlights
---------
• Strong config layer (schema validation, defaults, env overrides)
• Structured logging (console) + JSONL audit log (./logs/audit-YYYYMMDD.jsonl)
• Crash-safe backups (exports/) + auto-rotation (keep last 10)
• Health checks & diagnostics: `health`, `status`, `selftest`
• Built-in profiler for last command (`profile on|off`)
• Tool plugin loader (./tools/*.py → `Tool` subclasses auto-registered)
• Safer shell/file/web tools + request retry/backoff
• Planner v2 (rule-based + tiny keyword routing + multi-action support)
• Memory: short-term JSON + long-term SQLite/FTS (migrates if needed)
• Review improvements: filters by kind and time range
• Redaction & output compression for large logs (gzip chunks)

Stdlib-only by default. If `requests` is installed, WebTool uses it.
Drop-in replacement for your current francis.py. Keep your existing memory DB.
"""
...
class Francis:
    ...
    def plan(self, goal: str) -> List[Dict[str, Any]]:
        ...
        # multi-step parse: `run <cmd> && fetch <url>`
    ...
    def cmd_selftest(self):
        # Simple read/write, shell echo, web example
        ok = True
