...
# --- python resolver (deterministic) ---
$python = @(
    'C:\Francis\.venv\Scripts\python.exe',
    (Get-Command python3.exe -ErrorAction SilentlyContinue).Source,
    (Get-Command python.exe  -ErrorAction SilentlyContinue).Source
) | Where-Object { $_ -and (Test-Path $_) } | Select-Object -First 1
if ([string]::IsNullOrWhiteSpace($python)) { throw 'Python interpreter not found.' }
# --- end python resolver ---
$psi = New-Object System.Diagnostics.ProcessStartInfo
$psi.FileName = $python

if ([string]::IsNullOrWhiteSpace($psi.FileName)) { throw 'Python interpreter not found (psi filename blank).' }
...
if (Test-Path 'C:\Francis\.venv\Scripts\python.exe') {
$python = 'C:\Francis\.venv\Scripts\python.exe'
}
elseif (($cmd = Get-Command python.exe -ErrorAction SilentlyContinue) -and (Test-Path $cmd.Source)) {
$python = $cmd.Source
}
elseif (($cmd = Get-Command python3.exe -ErrorAction SilentlyContinue) -and (Test-Path $cmd.Source)) {
$python = $cmd.Source
}
else {
